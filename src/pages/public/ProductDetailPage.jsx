...
const ProductDetailPage = () => {
  ...
  // Auto-advance through images/videos every 4s; pauses on hover
  useEffect(() => {
    if (allMedia.length <= 1 || !autoPlay) return;
    const interval = setInterval(() => {
      setCurrentMediaIndex(prev => (prev + 1) % allMedia.length);
    }, 4000);
    return () => clearInterval(interval);
  }, [allMedia.length, autoPlay]);
        ...
        <div className="grid md:grid-cols-2 gap-8">
          ...
          <div className="bg-gray-900 rounded-2xl overflow-hidden">
            ...
            >
              ...
              {allMedia.length > 1 && (
                <>
                  ...
                  <button
                    ...
                    className="absolute right-3 top-1/2 -translate-y-1/2 bg-black/50 hover:bg-black/70 text-white rounded-full p-2 rotate-180 transition"
                  >
                    <ArrowLeft className="h-5 w-5" />
                  </button>
                </>
                  ...
                  {allMedia.map((_, idx) => (
                    <button
                      ...
                      className={`h-2 rounded-full transition-all ${
                        idx === currentMediaIndex ? 'bg-brand-orange w-6' : 'bg-gray-500 w-2'
                      }`}
                    />
                  ))}
                </div>
              ...
              {/* View profile — separate from image clicks */}
              {sellerId && (
                <Link
                  to={`/profile/${sellerId}`}
                  className="absolute bottom-4 right-4 bg-black/50 hover:bg-black/70 rounded-full px-3 py-1 text-xs text-white transition"
                >
                ...
                {allMedia.map((media, idx) => (
                  <button
                    ...
                    className={`flex-shrink-0 w-16 h-16 rounded-lg overflow-hidden border-2 transition ${
                      idx === currentMediaIndex ? 'border-brand-orange' : 'border-transparent opacity-60 hover:opacity-100'
                    }`}
                  >
                    {media.type === 'video' ? (
