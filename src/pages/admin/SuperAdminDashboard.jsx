...
// Icons cycle through these for plans beyond the first three, so a
// custom 4th/5th plan still gets something sensible instead of nothing.
const PLAN_ICONS = [Crown, Rocket, Star, Award, Target, Zap];
...
const SuperAdminDashboard = () => {
  ...
  // Fetch the real, current premium plans (name/price/duration/features)
  // from the backend instead of relying on hardcoded numbers. This is
  // what keeps prices shown here in sync with whatever was last saved
  // in Premium Plan management.
  const loadPremiumPlans = async () => {
    ...
    try {
      ...
      // Keep the selected plan in the Activate modal valid — if "basic"
      // no longer exists (deleted/renamed), fall back to the first plan.
  ...
  const handleActivatePremium = async () => {
    ...
    try {
      ...
      // Optimistically reflect the change immediately in the Recent Users
      // list, regardless of whether this user is still in the "first 10"
      // once we refetch (pagination/ordering shouldn't hide the update).
      setUsers(prev =>
        prev.map(u =>
      ...
      // Also bump the "Premium" stat card immediately so it doesn't look
      // stale while the real counts are refetched in the background.
      setStats(prev => ({
        ...prev,
        premiumUsers: selectedUserForPremium.is_premium
          ? prev.premiumUsers
          : prev.premiumUsers + 1,
      }));
      ...
      setShowPremiumModal(false);
      setSelectedUserForPremium(null);
      setPremiumPlan(premiumPlans[0]?.key || 'basic');
      loadData();
      loadPremiumData();
    } catch (error) {
      toast.error('Failed to activate premium');
  ...
  return (
    <div className="min-h-screen bg-[#f8f9fa] dark:bg-gray-900">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
        ...
        {/* Premium Management Tab — now driven by live plan data */}
        {activeTab === 'premium' && (
          <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-sm p-6">
            <div className="flex justify-between items-center mb-4">
              <h2 className="text-lg font-semibold text-gray-900 dark:text-white">Premium Management</h2>
            ...
            {plansLoading ? (
              ...
              <p className="text-center text-gray-500 dark:text-gray-400 py-10">
                No premium plans found. Create one from "Manage Premium Plans".
              </p>
            ) : (
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
                {premiumPlans.map((plan, idx) => {
                  const PlanIcon = PLAN_ICONS[idx % PLAN_ICONS.length];
                  // Middle plan (or 2nd of however many) gets the "Popular" highlight,
                  // matching the original design's treatment of the Standard plan.
                  const isPopular = idx === 1 && premiumPlans.length > 1;
                  ...
                  return (
                    ...
                    >
                      {isPopular && (
                        <div className="absolute -top-3 left-1/2 transform -translate-x-1/2 bg-brand-orange text-white text-xs px-2 py-0.5 rounded-full">
                          Popular
                        </div>
                      )}
                      {!plan.is_active && (
                        <div className="absolute -top-3 right-2 bg-gray-500 text-white text-xs px-2 py-0.5 rounded-full">
                          Inactive
                        </div>
                      )}
                      <PlanIcon className="h-8 w-8 text-yellow-500 mx-auto mb-2" />
                      ...
                      <ul className="mt-3 text-sm text-left space-y-1">
                        <li>✓ {plan.boost_multiplier}x view boost</li>
      ...
      {/* Premium Activation Modal — plan list now built from live data */}
      {showPremiumModal && selectedUserForPremium && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
          <div className="bg-white dark:bg-gray-800 rounded-2xl p-6 max-w-md w-full mx-4">
            <div className="flex justify-between items-center mb-4">
              ...
              <div>
                ...
                {plansLoading ? (
                  ...
                  <p className="text-sm text-gray-500 text-center py-4">
                    No premium plans available. Create one first.
                  </p>
                ) : (
                  <div className="space-y-2">
                    {premiumPlans
                          ...
                          <div className="flex items-center gap-3">
                            ...
                            <div>
                              ...
                              <p className="text-sm text-gray-500">
                                {plan.duration_days} days • {plan.boost_multiplier}x view boost
                              </p>
                            </div>
                          </div>
            ...
            <div className="flex space-x-3 mt-6">
              ...
              <button
                ...
                className="flex-1 px-4 py-2 bg-gradient-to-r from-yellow-500 to-orange-500 text-white rounded-xl font-semibold hover:from-yellow-600 hover:to-orange-600 transition flex items-center justify-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed"
              >
                <Crown className="h-4 w-4" />
                Activate Premium
              </button>
