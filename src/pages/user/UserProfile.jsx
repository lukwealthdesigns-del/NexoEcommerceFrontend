...
//   Package, ShoppingBag, TrendingUp, Crown,
//   ChevronRight, MapPin, Calendar, Mail, Phone, Edit2,
//   Plus
// } from 'lucide-react';
...
//     try {
//       const data = await usersService.getMyProducts();
//       setProducts(data || []);
//     } catch (error) {
...
//       // Update auth store (this persists across the app)
...
//       toast.error(error.response?.data?.detail || 'Failed to upload avatar', { id: toastId });
//     } finally {
...
//   const getAvatarUrl = () => {
//     // First check profile state
//     if (profile?.avatar_url) {
//       if (profile.avatar_url.startsWith('http')) return profile.avatar_url;
//       return `http://localhost:8080${profile.avatar_url}`;
//     }
//     // Then check user from auth store
//     if (user?.avatar_url) {
...
//   // Calculate total likes from all products
...
//                           <img 
//                             src={product.images?.[0] ? `http://localhost:8080${product.images[0]}` : 'https://via.placeholder.com/300x300?text=No+Image'} 
//                             alt={product.title} 
...
//                             </span>
//                             <span className="flex items-center gap-1">
//                               <MessageCircle className="h-3 w-3" /> {product.comments_count || 0}
//                             </span>
//                           </div>
//                         </div>
//                       </Link>
...
const UserProfile = () => {
  ...
  const loadUserProducts = async () => {
    try {
      ...
      if (data && Array.isArray(data)) {
        ...
        // Flatten the approved/pending/rejected
        productsList = [
          ...(data.approved || []),
          ...(data.pending || []),
          ...(data.rejected || [])
  ...
  const getAvatarUrl = () => {
    if (profile?.avatar_url) return getMediaUrl(profile.avatar_url);
  ...
  const ProductCard = ({ product, showLikeButton = false, showSaveButton = false, isLiked = false, isSaved = false }) => (
    <div className="group relative">
      <Link to={`/product/${product.id}`} className="block">
        <div className="aspect-square rounded-lg overflow-hidden bg-gray-100 dark:bg-gray-700">
          <img 
            src={getMediaUrl(product.images?.[0]) || 'https://via.placeholder.com/300x300?text=No+Image'} 
            alt={product.title} 
            className="w-full h-full object-cover group-hover:scale-105 transition duration-300"
            onError={(e) => {
              e.target.onerror = null;
